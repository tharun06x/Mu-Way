...
def validate_data(df: pd.DataFrame):
    """Validate required columns exist and have correct types/bounds (vectorized, fast for batch)."""
    ...
    if 'difficulty_level' in df.columns:
        # Accept floats (Bayesian-smoothed difficulty) — ensure they are in [1, 4]
    ...
    # Validate submission_date: parse and check for unparseable values.
    # Silent NaT propagation causes incorrect feature calculations downstream.
    parsed_dates = pd.to_datetime(df['submission_date'], errors='coerce')
    n_invalid = parsed_dates.isna().sum()
    if n_invalid > 0:
        bad_samples = df.loc[parsed_dates.isna(), 'submission_date'].head(5).tolist()
        raise ValueError(
            f"submission_date contains {n_invalid} unparseable value(s). "
            f"Sample bad values: {bad_samples}. Expected ISO format (YYYY-MM-DD)."
        )
...
def _mastery(group: pd.DataFrame, ref: pd.Timestamp) -> float:
    ...
    MasteryScore(u,d) = Σ[approved × diff × λ^days_ago]
                       / Σ[diff × λ^days_ago]
    """
    dates = pd.to_datetime(group['submission_date'])
...
def compute_user_features(
    ...
    if dates.dt.tz is not None:
        dates = dates.dt.tz_convert('UTC').dt.tz_localize(None)
    ref_naive = ref.replace(tzinfo=None) if ref.tzinfo is not None else ref
    feat['days_since_last_submission'] = max(0, int((ref_naive - dates.max()).days))  # BUG-2 fix: clamp to >= 0
...
def build_feature_store(
    ...
    if df['submission_date'].dt.tz is not None:
        df['submission_date'] = df['submission_date'].dt.tz_convert('UTC').dt.tz_localize(None)
    
    ref_naive = ref.replace(tzinfo=None) if ref.tzinfo is not None else ref
    df = df.dropna(subset=['submission_date'])
    df = df[df['submission_date'] <= ref_naive]
