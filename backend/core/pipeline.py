...
from core.features import compute_user_features, hashtag_to_domain, get_cold_start_features
...
from core.sequencer import build_roadmap_for_user
from core.utils import normalize_task_name as _normalize_task_name
...
RELATED_DOMAINS = {
    'ai':           ['data_science', 'dsa', 'web_backend'],
    'genai':        ['ai', 'data_science', 'web_backend'],
    'data_science': ['ai', 'data_analytics', 'dsa'],
    'data_analytics':['data_science', 'data_eng', 'business'],
    'data_eng':     ['data_science', 'devops', 'cloud'],
    'web_frontend': ['dsa', 'devops', 'ux'],
    'web_backend':  ['dsa', 'devops', 'cloud'],
    'mobile':       ['web_frontend', 'dsa'],
    'devops':       ['web_backend', 'cybersec', 'cloud'],
    'cybersec':     ['devops', 'web_backend', 'dsa'],
    'dsa':          ['web_backend', 'ai', 'data_science'],
    'cloud':        ['devops', 'web_backend', 'data_eng'],
    'ux':           ['web_frontend', 'product'],
    'product':      ['ux', 'business'],
    'business':     ['product', 'data_analytics'],
}
...
def spinner_task(label: str, func, *args, **kwargs):
    """Run func with a terminal spinner; return result or re-raise exception."""
    import threading
    ...
    i = 0
    while not done_flag.wait(timeout=0.08):
    ...
    if exc_holder[0] is not None:
        raise exc_holder[0]  # Propagate — don't silently return None
...
# _normalize_task_name moved to core.utils — imported at top of file as _normalize_task_name
...
def generate_roadmap(muid: str, name: str, role: str,
                     user_data: pd.DataFrame, task_data: pd.DataFrame,
                     task_features: pd.DataFrame = None) -> dict:
    """
    ...
    # Fix: role guard — fall back to Full Stack Developer if role unknown
    role = role if role in config.ROLE_REQUIREMENTS else 'Full Stack Developer'

    if known_user:
        rows['domain_mapped'] = rows['domain'].apply(hashtag_to_domain)
        features = compute_user_features(muid, rows, ref=pd.Timestamp.now())
    else:
        features = get_cold_start_features(muid)

    submitted_tasks = []
    if known_user:
        for _, row in rows.iterrows():
            if row.get('domain_mapped', 'general') == 'ignored':
                continue
            submitted_tasks.append({
                'task_name': row['task_name'],
                'domain': row.get('domain_mapped', 'general'),
                'difficulty_level': float(row.get('difficulty_level', 2.0)),
                'is_approved': int(row.get('is_approved', 0)),
    ...
    # Fix: use .get() to safely handle unknown roles
    role_reqs        = config.ROLE_REQUIREMENTS.get(role, config.ROLE_REQUIREMENTS['Full Stack Developer'])
    ...
    # Use pre-computed task_features if provided (avoids TF-IDF recompute per request)
    ...
    if not available.empty:
        ...
        def sort_key(row):
            ...
            # Push tasks below user's current level cleanly to the end
            # using 999.0 instead of float('inf') to avoid invalid JSON 'Infinity'
            return diff if diff >= next_diff else 999.0
        ...
        # Per-domain task cap: prevent any single domain flooding the pool.
        max_per_domain = getattr(config, 'MAX_TASKS_PER_DOMAIN', 4)
        available = (
            available
...
# Note: _cold_start_features removed — using get_cold_start_features from core.features
# which includes consistent Bayesian priors for per-domain approval_conf values.
