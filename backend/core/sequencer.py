...
from core.features import hashtag_to_domain
...
# _normalize_task_name is imported from core.utils (see imports above)
...
def _schedule_weeks(tasks: pd.DataFrame) -> list:
    ...
    while tasks_to_schedule and len(weeks) < MAX_WEEKS:
        ...
        if not current_week['tasks']:
            # Guard: if we made no progress (same tasks deferred as we started with),
            # break to prevent an infinite loop. This happens when ALL remaining tasks
            # are blocked by the domain cap but no single task exceeds the budget.
            if len(deferred_tasks) == len(tasks_to_schedule):
                break  # No progress was possible — avoid infinite loop
            # Also break if the first deferred task individually exceeds the weekly budget
            if deferred_tasks:
                first_mins = TASK_MINUTES.get(
                    str(deferred_tasks[0].get('complexity', 'Medium')).strip().capitalize(), 120
                )
                if first_mins > MINUTES_PER_WEEK:
                    break  # Task is individually too large to ever schedule
            break
