...
from core.features import hashtag_to_domain, _mastery
...
def engineer_user_features(user_data: pd.DataFrame) -> pd.DataFrame:
    ...
    Mastery is now computed using the canonical time-decayed, difficulty-weighted
    formula from core.features._mastery() — identical to what Problem 1 (feature
    store) and Problem 2 (career gap) use.  Previously it used a simplified
    sub_count / user_max ratio which caused the ML model to train on a different
    mastery signal than the gap calculator observed, leading to inconsistent
    recommendations.
    """
    records = []
    ...
    for (uid, dom), grp in user_data.groupby(['user_id', 'domain_mapped']):
        ...
        # Canonical mastery: time-decayed, difficulty-weighted (same as features.py)
        mastery = _mastery(grp, ref)
        ...
        records.append({
            ...
            'mastery':            mastery,
            'gap_score':          1.0 - mastery,          # gap is complement of mastery
...
def generate_labels(pairs: pd.DataFrame, user_data: pd.DataFrame) -> pd.DataFrame:
    """
    Task-intrinsic quality labels for the GBR training target.

    Previous approach: label = difficulty if user historically completed that exact
    task, else 0.  This had a data-leakage problem: whether a specific user ever
    submitted task X is known at training time but NOT at inference time.

    New approach: label = community_approval_rate × difficulty_level
      - community_approval_rate captures how broadly useful a task is across all users
      - difficulty_level weights harder (and thus more skill-building) tasks higher
      - Both values are known purely from task-level statistics, with zero leakage

    The label is normalised to [0, 4] to match the original scale so the
    GBR hyper-parameters (learning_rate, max_depth) don't need retuning.
    """
    # Build community approval rate per task: approved_count / total_count
    task_stats = (
        user_data
        .groupby('task_name')
    ...
    task_stats['community_approval_rate'] = (
        task_stats['approved'] / task_stats['total'].clip(lower=1)
    ).clip(0, 1)

    pairs = pairs.merge(
        task_stats[['task_name', 'community_approval_rate']],
        on='task_name',
        how='left',
    )
    pairs['community_approval_rate'] = pairs['community_approval_rate'].fillna(0.5)

    # Label: quality signal — harder widely-approved tasks score highest
    pairs['label'] = (
        pairs['community_approval_rate'] * pairs['difficulty_level'].astype(float)
    )

    # Normalise to [0, 4] to preserve the original GBR target scale
    mx = pairs['label'].max()
    if mx > 0:
        pairs['label'] = 4.0 * (pairs['label'] / mx)
...
class RankingModel:
    ...
    def save(self, path):
        """
        Persist model using joblib (safer and faster than pickle for numpy arrays).
        A SHA256 hash sidecar file (.sha256) is written alongside the model so
        RankingModel.load() can verify integrity before deserializing.
        """
        path = Path(path)
        joblib.dump(self, path)
        # Compute and store SHA256 hash for integrity verification on load
        digest = hashlib.sha256(path.read_bytes()).hexdigest()
        Path(str(path) + '.sha256').write_text(digest)
        logger.info(f'✓ Model saved → {path} (sha256={digest[:12]}…)')

    @staticmethod
    def load(path):
        """
        Load model with SHA256 integrity verification.
        Raises ValueError if the hash file exists and doesn't match,
        preventing Remote Code Execution from tampered model artifacts.
        """
        path = Path(path)
        ...
        if hash_file.exists():
            ...
            if expected != actual:
                raise ValueError(
                    ...
                    f"The file may have been tampered with. Re-train the model."
                )
            logger.info(f"Model integrity verified (sha256={actual[:12]}…)")
        else:
            logger.warning(
                f"No .sha256 sidecar found for {path}. "
                f"Skipping integrity check — consider re-saving the model to enable it."
            )
